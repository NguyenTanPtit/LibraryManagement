...
class EditBookActivity : BaseActivity() {
    ...
    override fun initViews() {
        ...
        if(bundle!=null){
            val book = bundle.getSerializable("book") as? BookDetailResponse
            if(book != null) {
                val author = Author(book.authorId.toString(), book.authorName?:"")
                val category = Category(book.categoryId?:0, book.categoryName?:"")
                bookUpdate = Book(book.id?:"", book.title?:"", book.cover?:"", book.state?:"", book.description?:"", author, category, book.price?:"", book.pageNumber?:"")
        ...
        if(requestCode == 1 && resultCode == RESULT_OK && data != null){
            ...
            if(imageUri != null) {
                val taskUpLoad = reference.child("book_cover")
                    .child(user?.id.toString()).putFile(imageUri)
                taskUpLoad.addOnSuccessListener {
                    taskUpLoad.continueWithTask { task ->
                        ...
                        reference.child("book_cover").child(user?.id.toString()).downloadUrl
